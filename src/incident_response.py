...
def log_incident(threat, status="Detected"):
    ...
    cursor.execute("INSERT INTO incident_logs (threat_name, response, timestamp, status) VALUES (?, ?, ?, ?)", 
                   (threat, response, timestamp, status))
    
    conn.commit()
    conn.close()
    
    return f"Incident logged: {threat} at {timestamp}"
